...
- Simple is the best
- Prefer functional programming rather than object oriented programming
  - Avoid creating custom class as much as possible
- Keep repository clean and well organized, it's last thing I want to do to put everything on the project root
- Prefer logging output to file rather than just showing on stdout
- No need for example code. Just make a module accept some argument for test run

# Project structure principal
- Create `code_doc` folder under project root
  - `code_doc/doc` for application specification, design documents
    - Refer documents under this folder when implementing
  - `code_doc/change_log` for change logs that describe what we've changed the code in each sessions
- Always create a module for managing environment variables, such as `env.py` or `env.ts`
  - The module checks if required environment variables are set or throw runtime error
  - Every other module MUST use this `env` module to access environment variables and MUST NOT directly use `process.env.XXX` equivalent
- Create `log` folder under project root
  - Unless requested, names of log correspond to modules emitting those logs
  - Log should be simple, minimal but cover what parts of modules succeed and/or fail. Avoid fancy looking styling
